...
public class StandardCheckDaoImp extends BaseDaoImp implements StandardCheckDao {
	...
	public List<TbAreaorgrelation> findAreaorgrelation(int orgId) {
		List result = null;
		try {
			String hql = "from TbAreaorgrelation where organizationNj = "
					+ orgId;
			result = getHibernateTemplate().find(hql);
		} catch (Exception e) {
			e.printStackTrace();
	...
	public List<TbSubareaorgrelation> findSubAreaId(Integer orgId) {
		List result = null;
		try {
			String hql = "from TbSubareaorgrelation where organizationNj = "
					+ orgId;
			result = getHibernateTemplate().find(hql);
		} catch (Exception e) {
			e.printStackTrace();
	 ...
	 * @return
	 */
	public boolean update(TbStandardcheck standardcheck) {
		...
		try {
			...
			TbStandardcheck load = (TbStandardcheck) openSession.load(
					TbStandardcheck.class, standardcheck.getStandardcheckid());
			load.setCheckscore(standardcheck.getCheckscore());
			load.setCreatedtime(standardcheck.getCheckdate());
