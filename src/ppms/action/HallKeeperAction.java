...
public class HallKeeperAction extends ActionSupport implements InitPage {
	...
	@Autowired
	private HallKeeperServiceImp hallkeeperserviceimp;
	...
	public Map<String, List<T>> initPage(ServletContext servletContext,
		...
		// 实例化map
				...
				case "userInfo.hallKeeperExamSearch":
					...
					//更具进阶内容查询进阶key
					List<TbMaster> tbMaster = serviceImp.getTbMaster("进阶店长");
					...
					for(TbPromotiontraining tpt:tbPromotiontraining){
						...
						String nowYear = String.valueOf(Integer.parseInt(sdf.format(nowdate))-1);
						//增强for查询出上一年的绩效集合
						...
						//封装数据进对象
						HallKeeperData hallKeeperData = new HallKeeperData(employeecode, employeename, areadesc, org_Name, postname, academicdegree, strtelecomlevel, strmobilelevel, performancescore, promotionscore, firstscore, secondscore, aver);
						//数据集添加对象
					...
					/*//更具区域id查询区域名称
					List<TbMaster> tbMaster = serviceImp.getTbMaster("进阶店长");				
					map.put("tbMaster", tbMaster);
					
					//取master的key值
					String promotioncontent = tbMaster.get(0).getKey();
					
					//根据key值查询进阶表信息
					List<TbPromotiontraining> tbPromotiontraining = serviceImp.getTbPromotiontraining(promotioncontent);
					map.put("tbPromotiontraining", tbPromotiontraining);
					
					
					//根据进阶表查询员工信息
					String employeeid = tbPromotiontraining.get(0).getTbEmployee().getEmployeeid();
					List<TbEmployee> tbEmployee = serviceImp.getTbEmployee(employeeid);
					map.put("tbEmployee", tbEmployee);
					
					//test
					List<TbEmployee> testList = null;
					for(TbPromotiontraining tpt:tbPromotiontraining){
						List<TbEmployee> TbEmployee = serviceImp.getTbEmployee(tpt.getTbEmployee().getEmployeeid());
						testList.add(TbEmployee.get(0));
					}
					...
					//营业厅id查询营业厅表
					List<OrganizationNj> organizationNj = serviceImp.getOrganizationNj(orgid);
					map.put("organizationNj", organizationNj);
					...
					//根据地域id查询地域集合
					List<TbArea> tbArea = serviceImp.getOrganizationNj(areaid);
					map.put("tbArea", tbArea);
					...
					//暗访信息集合查询
					List<TbVisitcheck> tbVisitcheck = serviceImp.getTbVisitcheck(orgid);
					map.put("tbVisitcheck", tbVisitcheck.get(0));*/
